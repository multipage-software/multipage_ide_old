...
public class XdebugClientOld {
	 ...
	 * @throws Exception
	 */
	public static XdebugClientOld connect(String ideHostName, int xdebugPort)
			throws Exception {
		
		// Create new Xdebug state.
		XdebugClientOld state = new XdebugClientOld();
		...
		while (!isConnected.ref && numAttempts > 0) {
			...
			}, XdebugListenerOld.xdebugClientTimeoutMs);
	 ...
	 * @throws Exception 
	 */
	synchronized public static XdebugClientOld connectNewClient(String ideHostName, int xdebugPort, String sourceLocator)
			throws Exception {
		...
		XdebugClientOld xdebugClient = XdebugClientOld.connect(ideHostName, xdebugPort);
		...
		String initialXmlPacket = String.format(
				...
				"fileuri=\"%s\"/>",
				processId,
				XdebugListenerOld.requiredIdeKey,
				threadId,
				sourceLocator);
					...
					if (key.isValid() && key.isReadable()) {
						...
						try {
							...
							if (responseText == null) {
								responseText = XdebugPacketOld.empty.toString();
							}
