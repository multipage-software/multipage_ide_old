...
/**
 * Xdebug listener is built-in socket server inside IDE.
 * @author user
 *
 */
public class XdebugListenerOld extends DebugListener {
	 ...
	 */
	private static XdebugListenerOld xdebugListenerSingleton;
	
	/**
	 * Gets the Xdebug client singleton
	 * @return
	 */
	public static XdebugListenerOld getSingleton() {
	...
	static class XdebugStatementOld {
		 ...
		 * Command response
		 */
		private XdebugPacketOld response = null;
	 ...
	 * Creates singleton
	 */
	public static XdebugListenerOld createInstance() throws Exception {
		
		// Use default Xdebug port number.
		xdebugListenerSingleton = new XdebugListenerOld(xdebugPort);
		return xdebugListenerSingleton;
	}
	 ...
	 * @return
	 */
	private XdebugPacketOld processReceivedCommand(String command) throws Exception {
	 ...
	 * @throws Exception 
	 */
	private XdebugPacketOld processReceivedCommandT(String commandName, String data) throws Exception {
	 ...
	 * @throws Exception 
	 */
	private XdebugPacketOld processReceivedCommand(String commandName, byte [] data) throws Exception {
		...
		// Get result
		XdebugPacketOld result = resultOf(command);
	...
	private XdebugPacketOld resultOf(XdebugStatementOld command) throws Exception {
		...
		if (command.response == null) {
			command.response = XdebugPacketOld.empty;
		}
	 ...
	 * Constructor
	 */
	public XdebugListenerOld(int port) throws Exception {
		...
		socketChannelThread = new Thread("IDE-Xdebug-Listener") {
			...
			private String getStatus(SocketChannel socketChannel) {
				...
				try {
					...
					if (Session.ready.equals(session)) {
						...
						beginTransaction();
						XdebugPacketOld responsePacket = passTo(socketChannel, "status", null);
			...
			private void initializeCommunication(SocketChannel connection) {
				...
				try {
					...
					// Initialization
					XdebugPacketOld packet = new XdebugPacketOld(message[1]);
	 ...
	 * @throws Exception
	 */
	public XdebugPacketOld runCommand(SocketChannel connection, String command) throws Exception {
		
		XdebugPacketOld responsePacket = command(connection, command);
	...
	protected String getSourceCode(SocketChannel connection, String fileUri) throws ChannelBreakDownException  {
		...
		beginTransaction();
		XdebugPacketOld responsePacket = command(connection, String.format("source -f %s", fileUri));
	 ...
	 * @return
	 */
	public XdebugPacketOld command(SocketChannel socketChannel, String command) throws ChannelBreakDownException {
	 ...
	 * @param data
	 */
	public XdebugPacketOld passTo(SocketChannel socketChannel, String command, byte [] data) throws ChannelBreakDownException {
