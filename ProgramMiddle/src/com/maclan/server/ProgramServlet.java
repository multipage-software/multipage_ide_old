...
public class ProgramServlet extends FastCGIServlet {
	...
	protected void handleArea(final HttpServletRequest _request, final HttpServletResponse _response)
		...
		com.maclan.server.Request request = new com.maclan.server.Request(_request, _request.getParameterMap());
		...
		if (developingWebApp(request)) {
			...
			if (login != null) {
				...
				if (result.isOK()) {
					...
					// Eventually exit the servlet without further response processing
					if (!processResponse) {
						return;
					}
